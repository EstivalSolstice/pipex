...
/*   Updated: 2024/12/01 15:13:30 by joltmann         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_whitespace_bonus(t_parser *parser)
{
	if (parser->arg_len > 0)
	{
		...
		parser->args = ft_realloc_array_bonus(parser->args, parser->args_count,
				parser->args_count + 1);
...
void	append_char_to_arg_bonus(t_parser *parser)
{
	...
	new_arg = ft_realloc_bonus(parser->arg, parser->arg_len, parser->arg_len
			+ 2);
	if (!new_arg)
		return ;
